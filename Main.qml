...
Item {
  ...
  function updateCommand(kind, agentIds) {
    // Keep the cloned widget's collector path user-owned. The stock Omarchy
    // updater currently passes an approval value that newer Codex CLIs reject;
    // bin/usage-update supplies the compatibility shim without editing stock.
    var command = ["bash", root.home + "/.config/omarchy/plugins/rafa.agents/bin/usage-update"]
