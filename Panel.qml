...
Panel {
  id: root
  moduleName: "rafa.agents"
  ipcTarget: "rafa.agents"
  manageIpc: false
  ...
  readonly property string fallbackIcon: "󱚣"
  ...
  function fiveHourWindow(p) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (windowSpanMs(label) === 5 * 3600 * 1000) {
        return {
  ...
  function barIconSource(p) {
    if (!p) return ""
    var id = String(p.providerId || "")
    var background = root.bar ? root.bar.background : Color.bar.background
    if (id === "codex" && colorLuminance(background) >= 0.5)
      return Qt.resolvedUrl("assets/codex-light.svg")
    return Qt.resolvedUrl("assets/" + id + ".svg")
  }

  function providerNames() {
    var names = []
    for (var i = 0; i < providers.length; i++) names.push(providerShortName(providers[i]))
    return names.join(" · ")
  }
  ...
  WidgetButton {
    ...
    anchors.fill: parent
    bar: root.bar
    text: root.vertical ? "󱚣" : root.barLabel
    labelVisible: false
    hasVisualContent: root.vertical || root.barLabel !== ""
    fontFamily: root.vertical ? "omarchy" : root.fontFamily
    fontSize: root.vertical ? Style.bar.iconFont : Style.font.bodySmall
    horizontalMargin: root.vertical ? 8.5 : 7
    verticalPadding: 5
    ...
    Row {
      ...
      Item {
        ...
        height: width
        transform: Translate { y: Style.space(2) }

        Image {
          id: barLogo
          anchors.fill: parent
          source: root.barIconSource(root.provider)
        ...
        Text {
          anchors.centerIn: parent
          visible: barLogo.status !== Image.Ready
          text: root.fallbackIcon
          color: button.active && button.useActiveColor ? button.activeColor : button.foreground
          font.family: "omarchy"
    ...
    // The text button must grow with the provider label instead of using the
    // fixed icon slot used by BarIconButton.
    fixedWidth: root.vertical ? -1 : barContent.implicitWidth + button.scaledHorizontalMargin * 2
  }
